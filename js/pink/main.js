...
gameMain.prototype = {
    preload: function(){ 
        ...
        allow_fish_overlap = true;
        phantom_powerUp = false;
    ...
    create: function(){
        ...
        phantom_islands = game.add.group();
        phantom_islands.enableBody = true;
        phantom_islands.physicsBodyType = Phaser.Physics.ARCADE;
        
        phantom_island = phantom_islands.create(-20, 518);
        phantom_island.width = island.width;
        phantom_island.height = island.height;
        phantom_island.body.immovable = true;
        ...
        //hero.animations.add('run', [0,1]);
        //hero.animations.add('dead', [6,7,8,9,10,11,12]);
        ...
        sea = game.add.tileSprite(0, HEIGHT-80, ROOM_WIDTH, 160, 'sea');
        game.add.tween(sea.tilePosition).to( { x: 800}, 40000,  Phaser.Easing.Sinusoidal.None, true, 0, -1);

        clouds = game.add.group();
        clouds.enableBody = true;
        clouds.physicsBodyType = Phaser.Physics.ARCADE;
        
        menuBtn = game.add.sprite(745, 15, 'menu');
        menuBtn.inputEnabled = true;
        menuBtn.scale.set(0.85, 0.85);
        menuBtn.alpha = 0.7;
        ...
        if (attr.currentLevel > PRETZEL_SCREEN) createPretzels();
        if (attr.currentLevel > ISLAND_SCREEN) createIslands();
        if (attr.currentLevel > CLOUD_SCREEN) createClouds();
        if (attr.currentLevel > BEE_SCREEN) setTimeout(function(){
            createBees();
        }, game.rnd.integerInRange(get_level_name().beeTime * 1.4, get_level_name().beeTime * 2));
        if (attr.currentLevel > FISH_SCREEN) createFishes();
        if (attr.currentLevel > RAIN_SCREEN){
            bg.frame = 1;
            create_rain();
        }

        finish_line = this.add.image(-156, 544, 'finish_line');
        finish_line.fixedToCamera= true;

        elephant_icon = this.add.image(100, 557, 'elephant');
        elephant_icon.scale.set(0.45, 0.45);

        tornado_icon = this.add.sprite(0, 545, 'tornado_sprite');
        ...
        tip = this.add.text(400, 250, get_level_name().introduce, {
        ...
        levelLabel = this.add.text(718, 535, get_level_name().name, {
            font: '20px ' + font, fill: 'orange', fontWeight: 'normal', align: 'center',
            stroke: 'purple', strokeThickness: 2
        });
        levelLabel.fixedToCamera = true;
        levelLabel.addColor('#e59400', 6);
        
        pauseLabel = this.add.text(230, 30, 'Game Paused\n Double tap to continue', {
            font: '30px ' + font, fill: 'purple', fontWeight: 'normal', align: 'center',
        ...
        best = this.add.sprite(720, 566, bestScore + 'star');
        best.scale.set(0.36, 0.36);
        ...
        musicFX = musics[attr.currentLevel-1];
        musicFX.play();
    },
    
    update: function(){  
        // adjust hero angle
        if (hero.angle < neededAngle){
            hero.body.angularVelocity += 0.05;  
        } 
        else if (hero.angle > neededAngle){
           hero.body.angularVelocity -= 0.05; 
        } 
        else{ 
        ...
        for (i=0; i < balloons.length; i++) {
            balloons.children[i].x = hero.body.x + (100 - (balloons.countDead() * 7)) + i;
            balloons.children[i].y = hero.body.y + 47;
            ...
            if (balloons.children[i].alive && balloons.children[i].scale.x < 0.43){ // balloon deflate
                ...
                hero.body.gravity.y += BASE_ADD_Y * 0.9;
                hero.body.gravity.x -= BASE_ADD_X / 2;  
        ...
        if (recordBroken && corrected_vel < HI_SPEED && speedLabel.text == ''){
            speedLabel = game.add.text(
                ...
                hero.body.y - 75, 
                HI_SPEED + " mph",        
                { font: '22px ' + font, fill: 'white', fontWeight: 'normal', align: 'center', stroke: '#000000',
                strokeThickness: 2 }
            );
        ...
        if (hero.body.y < -100) init_hero_attr('out');
        ...
        if (!phantom_powerUp){
            game.physics.arcade.collide(hero, phantom_islands, collide_hero_island, null, this);   
        }
        ...
        if (tornado_timer != null && !phantom_powerUp && hero.x > 112){ // collide only if user started playing
            game.physics.arcade.collide(hero, bees, collide_hero_bee, null, this);
            ...
            fishes.forEach(function(fish){
                if (isOverlapping(hero, fish) && allow_fish_overlap) overlap_fish_hero(hero, fish);
            });
            ...
            //if (isOverlapping(hero, sea)) overlap_sea_hero();
            ...
            pretzels.forEach(function(pretzel){
                ...
                phantom_islands.forEach(function(p_island){ // avoid overlapping pretzels-islands
                    if (isOverlapping(pretzel, p_island)){
                        p_island.x += 4; p_island.y += 4;
                    }
                });
            });
        }
...
function plus(){
    var overlap_fish_hero = false;
    
    fishes.forEach(function(fish){
        if (isOverlapping(hero, fish)) overlap_fish_hero = true;
    });
    
    if (count_balloons() < 6 && !overlap_fish_hero){
        
        if (attr.currentLevel == 1) modal.hideModal("new_game");
        
        rnd_n = attr.currentLevel;
        if (attr.currentLevel > 6) rnd_n = 6;
        rnd_baloon = game.rnd.integerInRange(0, rnd_n); 

        balloon = balloons.create(
            hero.body.x + (60 - (balloons.countDead()*7)),
            hero.body.y + 45,
            'balloon_sprite'
        );
        ...
        balloon.angle = -30 + 8 * ( count_balloons() * 1.5 );
...
function minus(bal_vel){
    ...
    if (balloons.length == 0 && !phantom_powerUp){
        hero.frame = 2;
    }    
}

function collide_hero_bee(_hero, _bee){
    var original_vel = _bee.body.velocity.x;
    _bee.body.velocity.x = 60;
    
    setTimeout(function(){
        if (original_vel < 0){
            _bee.body.velocity.x = original_vel;
        }
        else{
            _bee.body.velocity.x = -80;
        }
    }, 1400);
...
function collide_hero_island(_hero, _island){
    if (_hero.y < _island.y && _hero.x > _island.x){
        if (hero.body.velocity.x > 4){
            hero.body.velocity.x -= 4;
        }
        hero.angle = 7;
...
function overlap_fish_hero(_hero, _fish){  
   allow_fish_overlap = false;
   ...
   if (balloons.length != 0){
       hero.body.velocity.y -= 17; // gives a chance to survive the encounter
...
function overlap_pretzel_hero(_hero, _item) {
    ...
    else if (_item.key == 'pretzel_p'){
        _hero.frame = 3;
        phantom_powerUp = true;
        
        var time_left_p = 5;

        var tintInterval = setInterval(function(){
           time_left_p--; 
           if (time_left_p == 3) _hero.tint = 0xff0000;
           else if (time_left_p == 2) _hero.tint = 0xffffff;
           else if (time_left_p == 1) _hero.tint = 0xff0000;
        }, 1000);
...
function overlap_cloud_hero(_hero, _item) { 
   ...
   if (_item.frame == 0 || _item.frame == 1 || _item.frame == 2){
       hero.body.angularVelocity *= 2;
...
function kill_screen(reason){
   ...
   try{ clearTimeout(phantomTimer); } catch(e){}
   ...
   var time_left_round = null;
   var time_taken_round = null;
   var level = attr.currentLevel;
   var pretzels_percent = 0;
   var highest_speed = 0;
   var score = 0;
   //var distance_passed = Math.round(hero.x / ROOM_WIDTH) * 100 + '%';
   
   if (reason == 'end_game'){
       if (!lostFx.isPlaying) lostFx.play();  
       this.game.state.start("Game_over", false, false, score, reason, level, 0, 0, null, null);   
   }
   
   else if (reason == 'end_level'){ 
       end_levelFx.play();
       
       pretzels_percent = Math.round( (pretzels_taken / n_of_pretzels) * 100 ); 
       highest_speed = HI_SPEED; 
       ...
       time_taken_round = parseFloat(Math.round(time_taken * 10) / 10).toFixed(1);
       
       this.game.state.start(
           "Game_over", false, false,
           score, reason, level, pretzels_percent, highest_speed, time_taken_round,
...
function createFishes(){
    ...
    var meters_per_fish = ROOM_WIDTH / n_of_fishes;
    
    for(n=1; n < n_of_fishes; n++){
        var start_y = 580;
        var start_x = meters_per_fish * n + game.rnd.integerInRange(-150, 150); 
        var vel_y = game.rnd.integerInRange(-135, -350);
        var grav_y = game.rnd.integerInRange(65, 80);
        var scale = game.rnd.integerInRange(85, 115) / 100;

        fish = fishes.create(start_x, start_y, 'fish');
        //fish.tint = Math.random() * 0xffffff;
        
        fish.scale.set(scale, scale);
        fish.body.velocity.y = vel_y / scale;  
        fish.body.gravity.y = grav_y * scale; 
...
function createBees(){
    ...
    var start_y = game.rnd.integerInRange(40, 480);
    var start_x = game.rnd.integerInRange(WIDTH, ROOM_WIDTH);
    
    var vel_x = game.rnd.integerInRange(-40, -80) * (attr.currentLevel / 3) ;
    var scale = game.rnd.integerInRange(8, 12) / 10 ;

    bee = bees.create(hero.x + start_x, start_y, 'bee');
    bee.scale.set(scale, scale);
...
function createIslands(){
    n_of_islands = Math.round(ROOM_WIDTH / 950);
    
    var meters_per_island = ROOM_WIDTH / n_of_islands;
    ...
    for(n=0; n < n_of_islands; n++){
        ...
        var island_y;
        if (island_type != 1){
            island_y = game.rnd.integerInRange(60, 460); // make the islands fish collide with higher
        } 
        else { 
            island_y = game.rnd.integerInRange(60, 300); 
        }

        var island_x = meters_per_island * n + game.rnd.integerInRange(INIT_HERO_X + 100, ROOM_WIDTH - 250);
        ...
        phantom_island = phantom_islands.create(island_x, island_y+13);
        phantom_island.width = island.width;
        phantom_island.height = island.height-13;
        phantom_island.body.immovable = true;
...
function createPretzels(){
    ...
    for(n=0; n < n_of_pretzels; n++){
        ...
        var start_y = game.rnd.integerInRange(50, 300);
        var start_x = game.rnd.integerInRange(200, ROOM_WIDTH - 100); 

        var rnd = game.rnd.integerInRange(0,12);
        ...
        if (rnd > 4 && rnd != 9 && attr.currentLevel > 1){ // chance for time pizza = 7/12
           pretzel = pretzels.create(start_x, start_y, 'pretzel'); 
        } 
        else if (rnd <= 4 || attr.currentLevel == 1){ // chance for speed pizza = 5/12
            pretzel = pretzels.create(start_x, start_y, 'pretzel_s');
            pretzel.angle = -18;
        }
        else if (rnd == 9 && attr.currentLevel > 4){ // chance for phantom pizza = 1/12
            pretzel = pretzels.create(start_x, start_y, 'pretzel_p');
            pretzel.angle = -10;
        }
        try{
...
function createClouds(){
    var time_factor = attr.currentLevel * 500;
    var time_to_next = game.rnd.integerInRange(7500 - time_factor, 15000 - time_factor);
    
    var start_x = game.rnd.integerInRange(100, ROOM_WIDTH - 100);
    var start_y = game.rnd.integerInRange(20, 450);
    
    var rnd_cloud = game.rnd.integerInRange(0, 2 + (attr.currentLevel - 3));
    var rnd_scale = (game.rnd.integerInRange(6, 11)) / 10;

    var vel_x;
    if(rnd_cloud < 3){
       vel_x = game.rnd.integerInRange(50, 200); 
    }
    else{
       vel_x = game.rnd.integerInRange(-40, -170); 
    }
     
    var cloud_alpha = game.rnd.integerInRange(7, 10) / 10;

    cloud = clouds.create(start_x ,start_y, 'clouds');
    cloud.frame = rnd_cloud;
    
    cloud.body.velocity.x = vel_x;
    cloud.alpha = 0.1;
    game.add.tween(cloud).to( { alpha: cloud_alpha }, 1000, Phaser.Easing.Linear.None, true);
    var tween = game.add.tween(cloud.scale).to({ x: rnd_scale, y:rnd_scale }, 2500, Phaser.Easing.Linear.In, true);
    tween.yoyo(true, 250);
...
function save_score(score, time, percent, speed, time_taken){ // if it's the best score ever, save it to local storage
    ...
    var recordSpeed = '(Best: ' + bestSpeed + ')';
    var recordTimeTaken = '(Best: ' + bestTimeTaken + ')';
...
function createStartModal(){
    modal.createModal({
        ...
        itemsArr: [
            ...
            {
                type: "text",
                content: 'Swipe Right to add balloon\nSwipe Left to release balloon', 
                fontFamily: font,
                fontSize: 24,
                color: "0xF00E1F",
                offsetY: -85,
...
function loadSfx(){
    music1 = game.add.audio('music1', 0.9, true);
    music2 = game.add.audio('music2', 0.9, true);
    add_baloonFx = game.add.audio('add_baloon', 0.6, false);
    birdFx = game.add.audio('bird', 0.6, false);
    chewFx = game.add.audio('chew', 0.8, false);
    cloudFx = game.add.audio('cloud', 1, false);
    deflateFx = game.add.audio('deflate', 0.3, false);
    drowningFx = game.add.audio('drowning', 1, false);
    end_levelFx = game.add.audio('end_level', 0.4, false);
    fish_splashFx = game.add.audio('fish_splash', 1, false);
    lostFx = game.add.audio('lost', 1, false);
    outofboundsFx = game.add.audio('outofbounds', 1, false);
    pop_baloonFx = game.add.audio('pop_baloon', 0.7, false);
    pop_birdFx = game.add.audio('pop_bird', 1, false);

    musics = [music1, music2, music1, music2, music1, music2, music1, music2];
}
